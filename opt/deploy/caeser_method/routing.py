...
def make_map():
   ...
   # map.connect('user_database','/user/userdatabase/{id:.*}',controller = 'user', action = 'dbsearch')
	...
	m.connect('/dataset/add_button/{package_name}/{id}/{pack_r_id}/{user_name}',
                  action='add_button')
    ...
    with SubMapper(map, controller='user') as m:
        ...
        # in the route.
	m.connect('user_database','/user/userdatabase/{id:.*}',action='dbsearch')
	...
	m.connect('user_ffdatabase','/user111/userdatabase/{id:.*}',action='dbsearch')
